...
def main(argv):    
    ...
    print("Lista de argumentos no arquivo de entrada:")
    for i in range(len(entrada["configs"])):
        print(entrada["configs"][i])
    
    simulationName = entrada["configs"][0].get("name")
    nCsma = int(entrada["configs"][0].get("nCsma"))
    verbose = entrada["configs"][0].get("verbose")
    nWifi = int(entrada["configs"][0].get("nWifi"))
    tracing = entrada["configs"][0].get("tracing")
    simulationTime = float(entrada["configs"][0].get("time"))
    ...
    if tracing == "True":
    	pointToPoint.EnablePcapAll (simulationName)
    	phy.EnablePcap (simulationName, apDevices.Get (0))
    	csma.EnablePcap (simulationName, csmaDevices.Get (0), True)
